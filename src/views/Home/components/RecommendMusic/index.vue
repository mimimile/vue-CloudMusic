<template>
  <div class="recommend-music">
    <div class="recommend-music__songs">
      <subtitle title="推荐歌单"/>
      <div class="recommend-music__list">
        <div class="recommend-music__ul" v-for="(ul, index) in list" :key="index">
          <template v-for="item in ul">
            <recommend-cell class="recommend-music__li" :data="item" :key="item.id"></recommend-cell>
          </template>
          <div class="cb"></div>
        </div>
      </div>
    </div>
    <div class="recommend-music__newsg">
      <subtitle title="最新音乐"/>
      <div class="recommend-music__sglist" v-for="(item, index) in sglist" :key="index">
...
export default {
  ...
  methods: {
    ...
    async getSglist () {
      const { result } = await fetchSglist()
      this.sglist = result
    }
  }
}
...
@include b(recommend-music) {
  ...
  @include e(ft) {
    ...
    padding-top: 53.3%;
    margin-top: 4px;
    background: url(https://s3.music.126.net/m/s/img/recommand_bg_2x.png?d045fafc60e017b653f8065a87496922) no-repeat;
